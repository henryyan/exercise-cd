...
( function() {
	...
	function generateMarkup() { //build markup
		...
		newNode.innerHTML = [
			'<div class="header">',
				'<div class="left">',
					'<div id="', IDs.filters, '" class="filters" title="点击切换日志显示级别">', spans.join( '' ), '</div>',
				'</div>',
				'<div class="right">',
					'<div id="', IDs.controls, '" class="controls">',
						'<span id="', IDs.size ,'" title="contract" op="resize"></span>',
						'<span class="clear" title="清空" op="clear"></span>',
						'<span class="close" title="关闭" op="close"></span>',
					'</div>',
				'</div>',
			'</div>',
			'<div class="main">',
			...
			'</div>',
			'<div class="footer">',
				'<div class="left"><label for="', IDs.checkbox, '"><input type="checkbox" id="', IDs.checkbox, '" />默认显示日志</label></div>',
				'<div class="right"></div>',
			'</div>'
		].join( '' );
		return newNode;
