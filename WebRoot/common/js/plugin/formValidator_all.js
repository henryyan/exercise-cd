			...
			case "radio":
				...
				else
				{
					...
					if(stype =="date" || stype =="datetime"){
						var isok = false;
						if(ls_datatype=="date"){lb_go_on = isDate(val)};
						if(ls_datatype=="datetime"){lb_go_on = isDate(val)};
...
$.fn.formValidator = function(cs) 
{
	...
	return this.each(function(e)
	{
		...
		var stype = this.type;
		var defaultval = setting.defaultval;
		...
		if(srcTag == "input" || srcTag=="textarea")
		{
			...
			if(defaultval){
				if (stype == "checkbox" || stype == "radio") {
					if(stype == "radio"){
						if(this.value==defaultval){jqobj.attr("checked",true)}
					}
					else{
						jqobj.attr("checked",$.inArray(this.value, defaultval)>=0);
					}
				}else{
		...
		else if (srcTag == "select")
		{
			...
			if (defaultval){
				if(stype=="select-one"){
					...
					$("#"+this.id + " > option").each(function(){this.selected = ($.inArray(this.value, defaultval) >=0) });
				}
			}
			//获得焦点
